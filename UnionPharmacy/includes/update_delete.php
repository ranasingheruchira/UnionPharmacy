<?php
	...
	if(isset($_POST['update'])){
		...
		if (empty($pid) || empty($pname) || empty($ptype) || empty($pquan) || empty($pprice) || empty($file) || empty($description)) {
			...
			if(in_array($fileActualExt, $allowed)){
				if($fileError === 0){
					if($fileSize < 1000000){
						...
						$sql = "UPDATE product SET product_name='$pname', product_type='$ptype', quantity='$pquan', price='$pprice', 
